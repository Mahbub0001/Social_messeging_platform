export type Language = "bn" | "en";

export const translations = {
  bn: {
    // Nav & Sidebar
    messages: "বার্তা",
    feed: "ফিড",
    search: "খুঁজুন...",
    profileSettings: "প্রোফাইল সেটিংস",
    storyArchive: "স্টোরি আর্কাইভ",
    adminPanel: "অ্যাডমিন প্যানেল",
    signOut: "সাইন আউট",
    friends: "ফ্রেন্ডস",
    profile: "প্রোফাইল",
    language: "ভাষা / Language",
    selectLanguage: "অ্যাপের ভাষা পরিবর্তন করুন",
    bangla: "বাংলা",
    english: "English",
    theme: "থিম (Dark / Light)",
    
    // Feed
    communityFeed: "কমিউনিটি ফিড",
    allPosts: "সকল পোস্ট",
    myPosts: "আমার পোস্ট",
    createPostPlaceholder: "কী ভাবছেন, {name}? কিছু শেয়ার করুন...",
    post: "পোস্ট করুন",
    posting: "পোস্ট হচ্ছে...",
    react: "রিঅ্যাক্ট",
    comments: "মন্তব্য",
    repost: "রিশেয়ার",
    share: "শেয়ার করুন",
    repostToFeed: "ফিডে রিশেয়ার",
    sendToChat: "ইনবক্সে পাঠান",
    copyLink: "লিংক কপি করুন",
    reposted: "রিশেয়ার করা হয়েছে! ✓",
    linkCopied: "কপি করা হয়েছে! ✓",
    deletePost: "পোস্ট মুছুন",
    noPosts: "এখানে কোনো পোস্ট নেই",
    noComments: "এখনো কোনো মন্তব্য নেই। প্রথম মন্তব্য করুন! 💬",
    writeComment: "একটি মন্তব্য লিখুন...",
    photo: "ছবি",
    video: "ভিডিও",
    emoji: "ইমোজি",
    addPhoto: "ছবি যোগ করুন",
    addVideo: "ভিডিও যোগ করুন",
    chooseEmoji: "ইমোজি বেছে নিন",
    removePhoto: "ছবি সরান",
    removeVideo: "ভিডিও সরান",
    close: "বন্ধ করুন",
    moreOptions: "আরও বিকল্প",
    sendComment: "মন্তব্য পাঠান",
    sharePostTitle: "শেয়ার করুন",
    repostFeedDesc: "আপনার ফিডে শেয়ার করুন",
    sendToChatDesc: "কোনো কথোপকথনে পাঠান",
    noConversationsFound: "কোনো কথোপকথন পাওয়া যায়নি",
    sentCheck: "পাঠানো হয়েছে ✓",
    copyLinkDesc: "পোস্টের লিংক ক্লিপবোর্ডে কপি করুন",
    repostFailed: "রিশেয়ার ব্যর্থ হয়েছে। আবার চেষ্টা করুন।",
    postSharedLink: "পোস্ট শেয়ার",
    love: "ভালোবাসা",
    haha: "হা হা",
    wow: "অবাক",
    sad: "কষ্ট",
    angry: "রাগ",
    like: "পছন্দ",

    // Profile Settings
    usernameLabel: "ইউজারনেম",
    bioLabel: "বায়ো / পরিচয়",
    avatarUrlLabel: "অ্যাভেটার ছবি URL",
    saveChanges: "পরিবর্তন সংরক্ষণ করুন",
    saving: "সংরক্ষণ হচ্ছে...",
    yourStories: "আপনার স্টোরি সমূহ",
    profileSuccess: "প্রোফাইল সফলভাবে আপডেট করা হয়েছে!",
    friendsFeed: "বন্ধুদের পোস্ট",
    privacyPublic: "পাবলিক",
    privacyFriends: "শুধু বন্ধুরা",
    lockProfile: "প্রোফাইল লক",
    profileLocked: "প্রোফাইল লক করা",
    lockedProfileTitle: "{name} প্রোফাইল লক করেছেন",
    lockedProfileDesc: "শুধুমাত্র বন্ধুরা সম্পূর্ণ প্রোফাইল ও টাইমলাইন পোস্ট দেখতে পারেন।",
    addFriend: "অ্যাড ফ্রেন্ড",
    cancelRequest: "রিকোয়েস্ট বাতিল",
    requestSent: "রিকোয়েস্ট পাঠানো হয়েছে",
    unfriend: "আনফ্রেন্ড করুন",
    friendsCheck: "ফ্রেন্ডস ✓",
    message: "বার্তা পাঠান",
    sentRequestsTitle: "পাঠানো রিকোয়েস্ট",
    receivedRequestsTitle: "পেন্ডিং রিকোয়েস্ট",
    noSentRequests: "কোনো পেন্ডিং রিকোয়েস্ট পাঠানো নেই।",
    noReceivedRequests: "কোনো পেন্ডিং রিকোয়েস্ট নেই।",
    friendsAndRequests: "বন্ধুরা ও রিকোয়েস্ট",
    friendsTab: "বন্ধুরা",
    receivedTab: "আগত",
    sentTab: "পাঠানো",
    discoverTab: "অন্বেষণ",
    searchFriendsPlaceholder: "বন্ধুদের খুঁজুন...",
    addByUsername: "ইউজারনেম দিয়ে অ্যাড করুন",
    enterExactUsername: "সঠিক ইউজারনেম লিখুন...",
    sendRequestBtn: "রিকোয়েস্ট পাঠান",
    noFriendsFound: "কোনো বন্ধু পাওয়া যায়নি",
    noFriendsYet: "এখনো কোনো বন্ধু যুক্ত করেননি",
    findFriendsBtn: "নতুন বন্ধুদের খুঁজুন",
    discoverPeopleTitle: "সাজেস্টেড মানুষজন",
    noDiscoverPeople: "আপাতত নতুন কেউ নেই।",
    pendingStatus: "পেন্ডিং",
    chatBtn: "চ্যাট",
    profileBtn: "প্রোফাইল",
    acceptBtn: "গ্রহণ",
    declineBtn: "মুছুন",
    cancelSentReqBtn: "বাতিল",

    // Time
    justNow: "এইমাত্র",
    minsAgo: "{n} মিনিট আগে",
    hoursAgo: "{n} ঘণ্টা আগে",
    yesterday: "গতকাল"
  },
  en: {
    // Nav & Sidebar
    messages: "Messages",
    feed: "Feed",
    search: "Search...",
    profileSettings: "Profile Settings",
    storyArchive: "Story Archive",
    adminPanel: "Admin Panel",
    signOut: "Sign Out",
    friends: "Friends",
    profile: "Profile",
    language: "Language",
    selectLanguage: "Change App Language",
    bangla: "বাংলা",
    english: "English",
    theme: "Theme (Dark / Light)",

    // Feed
    communityFeed: "Community Feed",
    allPosts: "All Posts",
    myPosts: "My Posts",
    friendsFeed: "Friends",
    privacyPublic: "Public",
    privacyFriends: "Friends Only",
    lockProfile: "Lock Profile",
    profileLocked: "Profile Locked",
    lockedProfileTitle: "{name} locked their profile",
    lockedProfileDesc: "Only friends can see full profile details and timeline posts.",
    addFriend: "Add Friend",
    cancelRequest: "Cancel Request",
    requestSent: "Request Sent",
    unfriend: "Unfriend",
    friendsCheck: "Friends ✓",
    message: "Message",
    sentRequestsTitle: "Sent Requests",
    receivedRequestsTitle: "Pending Requests",
    noSentRequests: "No pending sent requests.",
    noReceivedRequests: "No pending invitations.",
    friendsAndRequests: "Friends & Requests",
    friendsTab: "Friends",
    receivedTab: "Received",
    sentTab: "Sent",
    discoverTab: "Discover",
    searchFriendsPlaceholder: "Search friends...",
    addByUsername: "Add Friend by Username",
    enterExactUsername: "Enter exact username...",
    sendRequestBtn: "Send Request",
    noFriendsFound: "No friends found",
    noFriendsYet: "No friends added yet",
    findFriendsBtn: "Find People to Add",
    discoverPeopleTitle: "Suggested People",
    noDiscoverPeople: "No new people to discover right now.",
    pendingStatus: "Pending",
    chatBtn: "Chat",
    profileBtn: "Profile",
    acceptBtn: "Accept",
    declineBtn: "Decline",
    cancelSentReqBtn: "Cancel",
    createPostPlaceholder: "What's on your mind, {name}? Share something...",
    post: "Post",
    posting: "Posting...",
    react: "React",
    comments: "Comments",
    repost: "Repost",
    share: "Share",
    repostToFeed: "Repost to Feed",
    sendToChat: "Send to Chat",
    copyLink: "Copy Link",
    reposted: "Reposted! ✓",
    linkCopied: "Link Copied! ✓",
    deletePost: "Delete Post",
    noPosts: "No posts here yet",
    noComments: "No comments yet. Be the first to comment! 💬",
    writeComment: "Write a comment...",
    photo: "Photo",
    video: "Video",
    emoji: "Emoji",
    addPhoto: "Add Photo",
    addVideo: "Add Video",
    chooseEmoji: "Choose emoji",
    removePhoto: "Remove photo",
    removeVideo: "Remove video",
    close: "Close",
    moreOptions: "More options",
    sendComment: "Send comment",
    sharePostTitle: "Share",
    repostFeedDesc: "Share to your feed",
    sendToChatDesc: "Send to a conversation",
    noConversationsFound: "No conversations found",
    sentCheck: "Sent ✓",
    copyLinkDesc: "Copy post link to clipboard",
    repostFailed: "Repost failed. Please try again.",
    postSharedLink: "Shared post",
    love: "Love",
    haha: "Haha",
    wow: "Wow",
    sad: "Sad",
    angry: "Angry",
    like: "Like",

    // Profile Settings
    usernameLabel: "Username",
    bioLabel: "Bio",
    avatarUrlLabel: "Avatar Image URL",
    saveChanges: "Save Changes",
    saving: "Saving...",
    yourStories: "Your Stories",
    profileSuccess: "Profile updated successfully!",

    // Time
    justNow: "Just now",
    minsAgo: "{n}m ago",
    hoursAgo: "{n}h ago",
    yesterday: "Yesterday"
  }
};

export function getTranslation(lang: Language, key: keyof typeof translations.bn, params?: Record<string, string | number>): string {
  let str = translations[lang]?.[key] || translations.bn[key] || String(key);
  if (params) {
    Object.entries(params).forEach(([k, v]) => {
      str = str.replace(new RegExp(`\\{${k}\\}`, 'g'), String(v));
    });
  }
  return str;
}
